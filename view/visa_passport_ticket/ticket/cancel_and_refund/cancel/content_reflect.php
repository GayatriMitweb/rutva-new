...
$sq_payment_info = mysql_fetch_assoc(mysql_query("select sum(payment_amount) as sum from ticket_payment_master where ticket_id='$ticket_id' AND clearance_status!='Pending' AND clearance_status!='Cancelled'"));
...
<div class="row">
	<div class="col-xs-12 no-pad"> <div class="table-responsive">
		<table class="table table-bordered table-hover mg_bt_0">
            ...
            $count = 0;
            $disabled_count= 0;
			$sq_ticket_entries = mysql_query("select * from ticket_master_entries where ticket_id='$ticket_id'");
...
<?php 
$sq_cancel_count = mysql_num_rows(mysql_query("select * from ticket_master_entries where ticket_id='$ticket_id' and status='Cancel'"));
if($sq_cancel_count>0){
    $sq_ticket_info = mysql_fetch_assoc(mysql_query("select * from ticket_master where ticket_id='$ticket_id'"));
?>
<form id="frm_refund">

	<div class="row text-center mg_tp_10 mg_bt_30">
		...
		<div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10_xs">
			<input type="text" name="total_refund_amount" id="total_refund_amount" class="amount_feild_highlight text-right" placeholder="Total Refund" title="Total Refund" readonly value="<?= $sq_ticket_info['total_refund_amount'] ?>">
		</div>
	</div>
	<?php if($sq_ticket_info['cancel_amount'] == "0.00"){ ?>
	<div class="row mg_tp_20">
